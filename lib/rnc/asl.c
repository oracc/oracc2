#ifndef NULL
#define NULL (char *)0
#endif
#include "xnn.h"
#include "ns-asl.h"
struct xnn_nstab asl_nstab[] = {
  { asl_n_g, "xmlns:g", "http://oracc.org/ns/gdl/1.0" },
  { asl_n_n, "xmlns:n", "http://oracc.org/ns/norm/1.0" },
  { asl_n_sl, "xmlns:sl", "http://oracc.org/ns/sl/1.0" },
  { asl_n_syn, "xmlns:syn", "http://oracc.org/ns/syntax/1.0" },
  { asl_n_xh, "xmlns:xh", "http://www.w3.org/1999/xhtml" },
  { asl_n_xtr, "xmlns:xtr", "http://oracc.org/ns/xtr/1.0" },
  { asl_a_enum_top, NULL, NULL },
};

struct xnn_xname asl_anames[] =
{
  { "xmlns:g", "xmlns:g" },
  { "xmlns:n", "xmlns:n" },
  { "xmlns:sl", "xmlns:sl" },
  { "xmlns:syn", "xmlns:syn" },
  { "xmlns:xh", "xmlns:xh" },
  { "xmlns:xml", "xmlns:xml" },
  { "xmlns:xtr", "xmlns:xtr" },
  { "base", "base" },
  { "bcnt", "bcnt" },
  { "class", "class" },
  { "comment", "comment" },
  { "compoundonly", "compoundonly" },
  { "contrefs", "contrefs" },
  { "count", "count" },
  { "cpd-refs", "cpd-refs" },
  { "ctotal", "ctotal" },
  { "deprecated", "deprecated" },
  { "dir", "dir" },
  { "form", "form" },
  { "fwhost", "fwhost" },
  { "g:accented", "http://oracc.org/ns/gdl/1.0:accented" },
  { "g:break", "http://oracc.org/ns/gdl/1.0:break" },
  { "g:breakEnd", "http://oracc.org/ns/gdl/1.0:breakEnd" },
  { "g:breakStart", "http://oracc.org/ns/gdl/1.0:breakStart" },
  { "g:c", "http://oracc.org/ns/gdl/1.0:c" },
  { "g:collated", "http://oracc.org/ns/gdl/1.0:collated" },
  { "g:damageEnd", "http://oracc.org/ns/gdl/1.0:damageEnd" },
  { "g:damageStart", "http://oracc.org/ns/gdl/1.0:damageStart" },
  { "g:delim", "http://oracc.org/ns/gdl/1.0:delim" },
  { "g:em", "http://oracc.org/ns/gdl/1.0:em" },
  { "g:hc", "http://oracc.org/ns/gdl/1.0:hc" },
  { "g:ho", "http://oracc.org/ns/gdl/1.0:ho" },
  { "g:logolang", "http://oracc.org/ns/gdl/1.0:logolang" },
  { "g:o", "http://oracc.org/ns/gdl/1.0:o" },
  { "g:pos", "http://oracc.org/ns/gdl/1.0:pos" },
  { "g:prox", "http://oracc.org/ns/gdl/1.0:prox" },
  { "g:queried", "http://oracc.org/ns/gdl/1.0:queried" },
  { "g:remarked", "http://oracc.org/ns/gdl/1.0:remarked" },
  { "g:role", "http://oracc.org/ns/gdl/1.0:role" },
  { "g:script", "http://oracc.org/ns/gdl/1.0:script" },
  { "g:sign", "http://oracc.org/ns/gdl/1.0:sign" },
  { "g:status", "http://oracc.org/ns/gdl/1.0:status" },
  { "g:statusEnd", "http://oracc.org/ns/gdl/1.0:statusEnd" },
  { "g:statusStart", "http://oracc.org/ns/gdl/1.0:statusStart" },
  { "g:surroEnd", "http://oracc.org/ns/gdl/1.0:surroEnd" },
  { "g:surroStart", "http://oracc.org/ns/gdl/1.0:surroStart" },
  { "g:type", "http://oracc.org/ns/gdl/1.0:type" },
  { "g:uflag1", "http://oracc.org/ns/gdl/1.0:uflag1" },
  { "g:uflag2", "http://oracc.org/ns/gdl/1.0:uflag2" },
  { "g:uflag3", "http://oracc.org/ns/gdl/1.0:uflag3" },
  { "g:uflag4", "http://oracc.org/ns/gdl/1.0:uflag4" },
  { "g:utf8", "http://oracc.org/ns/gdl/1.0:utf8" },
  { "g:varc", "http://oracc.org/ns/gdl/1.0:varc" },
  { "g:vari", "http://oracc.org/ns/gdl/1.0:vari" },
  { "g:varo", "http://oracc.org/ns/gdl/1.0:varo" },
  { "gpos", "gpos" },
  { "guide", "guide" },
  { "headform", "headform" },
  { "headref", "headref" },
  { "hex", "hex" },
  { "hid", "hid" },
  { "icnt", "icnt" },
  { "implicit", "implicit" },
  { "ipct", "ipct" },
  { "label", "label" },
  { "lcnt", "lcnt" },
  { "lemma", "lemma" },
  { "lid", "lid" },
  { "loc", "loc" },
  { "merge", "merge" },
  { "moid", "moid" },
  { "morph", "morph" },
  { "n", "n" },
  { "n:num", "http://oracc.org/ns/norm/1.0:num" },
  { "name", "name" },
  { "norm", "norm" },
  { "num", "num" },
  { "o", "o" },
  { "oid", "oid" },
  { "oids", "oids" },
  { "order", "order" },
  { "p", "p" },
  { "path", "path" },
  { "period", "period" },
  { "pos", "pos" },
  { "proj", "proj" },
  { "project", "project" },
  { "qm", "qm" },
  { "qv", "qv" },
  { "ref", "ref" },
  { "sense", "sense" },
  { "sexified", "sexified" },
  { "signlist", "signlist" },
  { "sort", "sort" },
  { "subname", "subname" },
  { "swc-final", "swc-final" },
  { "syn:brk-after", "http://oracc.org/ns/syntax/1.0:brk-after" },
  { "syn:brk-before", "http://oracc.org/ns/syntax/1.0:brk-before" },
  { "syn:ub-after", "http://oracc.org/ns/syntax/1.0:ub-after" },
  { "syn:ub-before", "http://oracc.org/ns/syntax/1.0:ub-before" },
  { "thumb", "thumb" },
  { "tid", "tid" },
  { "title", "title" },
  { "token", "token" },
  { "type", "type" },
  { "uncertain", "uncertain" },
  { "v", "v" },
  { "xml:id", "http://www.w3.org/XML/1998/namespace:id" },
  { "xml:lang", "http://www.w3.org/XML/1998/namespace:lang" },
  { "xtr:cid", "http://oracc.org/ns/xtr/1.0:cid" },
  { "xtr:code", "http://oracc.org/ns/xtr/1.0:code" },
  { "xtr:cols", "http://oracc.org/ns/xtr/1.0:cols" },
  { "xtr:disamb", "http://oracc.org/ns/xtr/1.0:disamb" },
  { "xtr:eref", "http://oracc.org/ns/xtr/1.0:eref" },
  { "xtr:form", "http://oracc.org/ns/xtr/1.0:form" },
  { "xtr:hdr-ref", "http://oracc.org/ns/xtr/1.0:hdr-ref" },
  { "xtr:lab-end-label", "http://oracc.org/ns/xtr/1.0:lab-end-label" },
  { "xtr:lab-end-lnum", "http://oracc.org/ns/xtr/1.0:lab-end-lnum" },
  { "xtr:lab-start-label", "http://oracc.org/ns/xtr/1.0:lab-start-label" },
  { "xtr:lab-start-lnum", "http://oracc.org/ns/xtr/1.0:lab-start-lnum" },
  { "xtr:label", "http://oracc.org/ns/xtr/1.0:label" },
  { "xtr:lem", "http://oracc.org/ns/xtr/1.0:lem" },
  { "xtr:nrefs", "http://oracc.org/ns/xtr/1.0:nrefs" },
  { "xtr:overlap", "http://oracc.org/ns/xtr/1.0:overlap" },
  { "xtr:ref", "http://oracc.org/ns/xtr/1.0:ref" },
  { "xtr:refs", "http://oracc.org/ns/xtr/1.0:refs" },
  { "xtr:rend-label", "http://oracc.org/ns/xtr/1.0:rend-label" },
  { "xtr:rows", "http://oracc.org/ns/xtr/1.0:rows" },
  { "xtr:se_label", "http://oracc.org/ns/xtr/1.0:se_label" },
  { "xtr:silent", "http://oracc.org/ns/xtr/1.0:silent" },
  { "xtr:span", "http://oracc.org/ns/xtr/1.0:span" },
  { "xtr:spanall", "http://oracc.org/ns/xtr/1.0:spanall" },
  { "xtr:sref", "http://oracc.org/ns/xtr/1.0:sref" },
  { "xtr:standalone", "http://oracc.org/ns/xtr/1.0:standalone" },
  { "xtr:type", "http://oracc.org/ns/xtr/1.0:type" },
  { "xtr:unit", "http://oracc.org/ns/xtr/1.0:unit" },
  { "xtr:uref", "http://oracc.org/ns/xtr/1.0:uref" },
  { NULL, NULL },
};
struct xnn_attr asl_abases[] =
{
  { { "base",NULL } , { "base",NULL } },
  { { "bcnt",NULL } , { "bcnt",NULL } },
  { { "class",NULL } , { "class",NULL } },
  { { "comment",NULL } , { "comment",NULL } },
  { { "compoundonly",NULL } , { "compoundonly",NULL } },
  { { "contrefs",NULL } , { "contrefs",NULL } },
  { { "count",NULL } , { "count",NULL } },
  { { "cpd-refs",NULL } , { "cpd-refs",NULL } },
  { { "ctotal",NULL } , { "ctotal",NULL } },
  { { "deprecated",NULL } , { "deprecated",NULL } },
  { { "dir",NULL } , { "dir",NULL } },
  { { "form",NULL } , { "form",NULL } },
  { { "fwhost",NULL } , { "fwhost",NULL } },
  { { "g:accented",NULL } , { "http://oracc.org/ns/gdl/1.0:accented",NULL } },
  { { "g:break",NULL } , { "http://oracc.org/ns/gdl/1.0:break",NULL } },
  { { "g:breakEnd",NULL } , { "http://oracc.org/ns/gdl/1.0:breakEnd",NULL } },
  { { "g:breakStart",NULL } , { "http://oracc.org/ns/gdl/1.0:breakStart",NULL } },
  { { "g:c",NULL } , { "http://oracc.org/ns/gdl/1.0:c",NULL } },
  { { "g:collated",NULL } , { "http://oracc.org/ns/gdl/1.0:collated",NULL } },
  { { "g:damageEnd",NULL } , { "http://oracc.org/ns/gdl/1.0:damageEnd",NULL } },
  { { "g:damageStart",NULL } , { "http://oracc.org/ns/gdl/1.0:damageStart",NULL } },
  { { "g:delim",NULL } , { "http://oracc.org/ns/gdl/1.0:delim",NULL } },
  { { "g:em",NULL } , { "http://oracc.org/ns/gdl/1.0:em",NULL } },
  { { "g:hc",NULL } , { "http://oracc.org/ns/gdl/1.0:hc",NULL } },
  { { "g:ho",NULL } , { "http://oracc.org/ns/gdl/1.0:ho",NULL } },
  { { "g:logolang",NULL } , { "http://oracc.org/ns/gdl/1.0:logolang",NULL } },
  { { "g:o",NULL } , { "http://oracc.org/ns/gdl/1.0:o",NULL } },
  { { "g:pos",NULL } , { "http://oracc.org/ns/gdl/1.0:pos",NULL } },
  { { "g:prox",NULL } , { "http://oracc.org/ns/gdl/1.0:prox",NULL } },
  { { "g:queried",NULL } , { "http://oracc.org/ns/gdl/1.0:queried",NULL } },
  { { "g:remarked",NULL } , { "http://oracc.org/ns/gdl/1.0:remarked",NULL } },
  { { "g:role",NULL } , { "http://oracc.org/ns/gdl/1.0:role",NULL } },
  { { "g:script",NULL } , { "http://oracc.org/ns/gdl/1.0:script",NULL } },
  { { "g:sign",NULL } , { "http://oracc.org/ns/gdl/1.0:sign",NULL } },
  { { "g:status",NULL } , { "http://oracc.org/ns/gdl/1.0:status",NULL } },
  { { "g:statusEnd",NULL } , { "http://oracc.org/ns/gdl/1.0:statusEnd",NULL } },
  { { "g:statusStart",NULL } , { "http://oracc.org/ns/gdl/1.0:statusStart",NULL } },
  { { "g:surroEnd",NULL } , { "http://oracc.org/ns/gdl/1.0:surroEnd",NULL } },
  { { "g:surroStart",NULL } , { "http://oracc.org/ns/gdl/1.0:surroStart",NULL } },
  { { "g:type",NULL } , { "http://oracc.org/ns/gdl/1.0:type",NULL } },
  { { "g:uflag1",NULL } , { "http://oracc.org/ns/gdl/1.0:uflag1",NULL } },
  { { "g:uflag2",NULL } , { "http://oracc.org/ns/gdl/1.0:uflag2",NULL } },
  { { "g:uflag3",NULL } , { "http://oracc.org/ns/gdl/1.0:uflag3",NULL } },
  { { "g:uflag4",NULL } , { "http://oracc.org/ns/gdl/1.0:uflag4",NULL } },
  { { "g:utf8",NULL } , { "http://oracc.org/ns/gdl/1.0:utf8",NULL } },
  { { "g:varc",NULL } , { "http://oracc.org/ns/gdl/1.0:varc",NULL } },
  { { "g:vari",NULL } , { "http://oracc.org/ns/gdl/1.0:vari",NULL } },
  { { "g:varo",NULL } , { "http://oracc.org/ns/gdl/1.0:varo",NULL } },
  { { "gpos",NULL } , { "gpos",NULL } },
  { { "guide",NULL } , { "guide",NULL } },
  { { "headform",NULL } , { "headform",NULL } },
  { { "headref",NULL } , { "headref",NULL } },
  { { "hex",NULL } , { "hex",NULL } },
  { { "hid",NULL } , { "hid",NULL } },
  { { "icnt",NULL } , { "icnt",NULL } },
  { { "implicit",NULL } , { "implicit",NULL } },
  { { "ipct",NULL } , { "ipct",NULL } },
  { { "label",NULL } , { "label",NULL } },
  { { "lcnt",NULL } , { "lcnt",NULL } },
  { { "lemma",NULL } , { "lemma",NULL } },
  { { "lid",NULL } , { "lid",NULL } },
  { { "loc",NULL } , { "loc",NULL } },
  { { "merge",NULL } , { "merge",NULL } },
  { { "moid",NULL } , { "moid",NULL } },
  { { "morph",NULL } , { "morph",NULL } },
  { { "n",NULL } , { "n",NULL } },
  { { "n:num",NULL } , { "http://oracc.org/ns/norm/1.0:num",NULL } },
  { { "name",NULL } , { "name",NULL } },
  { { "norm",NULL } , { "norm",NULL } },
  { { "num",NULL } , { "num",NULL } },
  { { "o",NULL } , { "o",NULL } },
  { { "oid",NULL } , { "oid",NULL } },
  { { "oids",NULL } , { "oids",NULL } },
  { { "order",NULL } , { "order",NULL } },
  { { "p",NULL } , { "p",NULL } },
  { { "path",NULL } , { "path",NULL } },
  { { "period",NULL } , { "period",NULL } },
  { { "pos",NULL } , { "pos",NULL } },
  { { "proj",NULL } , { "proj",NULL } },
  { { "project",NULL } , { "project",NULL } },
  { { "qm",NULL } , { "qm",NULL } },
  { { "qv",NULL } , { "qv",NULL } },
  { { "ref",NULL } , { "ref",NULL } },
  { { "sense",NULL } , { "sense",NULL } },
  { { "sexified",NULL } , { "sexified",NULL } },
  { { "signlist",NULL } , { "signlist",NULL } },
  { { "sort",NULL } , { "sort",NULL } },
  { { "subname",NULL } , { "subname",NULL } },
  { { "swc-final",NULL } , { "swc-final",NULL } },
  { { "syn:brk-after",NULL } , { "http://oracc.org/ns/syntax/1.0:brk-after",NULL } },
  { { "syn:brk-before",NULL } , { "http://oracc.org/ns/syntax/1.0:brk-before",NULL } },
  { { "syn:ub-after",NULL } , { "http://oracc.org/ns/syntax/1.0:ub-after",NULL } },
  { { "syn:ub-before",NULL } , { "http://oracc.org/ns/syntax/1.0:ub-before",NULL } },
  { { "thumb",NULL } , { "thumb",NULL } },
  { { "tid",NULL } , { "tid",NULL } },
  { { "title",NULL } , { "title",NULL } },
  { { "token",NULL } , { "token",NULL } },
  { { "type",NULL } , { "type",NULL } },
  { { "uncertain",NULL } , { "uncertain",NULL } },
  { { "v",NULL } , { "v",NULL } },
  { { "xml:id",NULL } , { "http://www.w3.org/XML/1998/namespace:id",NULL } },
  { { "xml:lang",NULL } , { "http://www.w3.org/XML/1998/namespace:lang",NULL } },
  { { "xtr:cid",NULL } , { "http://oracc.org/ns/xtr/1.0:cid",NULL } },
  { { "xtr:code",NULL } , { "http://oracc.org/ns/xtr/1.0:code",NULL } },
  { { "xtr:cols",NULL } , { "http://oracc.org/ns/xtr/1.0:cols",NULL } },
  { { "xtr:disamb",NULL } , { "http://oracc.org/ns/xtr/1.0:disamb",NULL } },
  { { "xtr:eref",NULL } , { "http://oracc.org/ns/xtr/1.0:eref",NULL } },
  { { "xtr:form",NULL } , { "http://oracc.org/ns/xtr/1.0:form",NULL } },
  { { "xtr:hdr-ref",NULL } , { "http://oracc.org/ns/xtr/1.0:hdr-ref",NULL } },
  { { "xtr:lab-end-label",NULL } , { "http://oracc.org/ns/xtr/1.0:lab-end-label",NULL } },
  { { "xtr:lab-end-lnum",NULL } , { "http://oracc.org/ns/xtr/1.0:lab-end-lnum",NULL } },
  { { "xtr:lab-start-label",NULL } , { "http://oracc.org/ns/xtr/1.0:lab-start-label",NULL } },
  { { "xtr:lab-start-lnum",NULL } , { "http://oracc.org/ns/xtr/1.0:lab-start-lnum",NULL } },
  { { "xtr:label",NULL } , { "http://oracc.org/ns/xtr/1.0:label",NULL } },
  { { "xtr:lem",NULL } , { "http://oracc.org/ns/xtr/1.0:lem",NULL } },
  { { "xtr:nrefs",NULL } , { "http://oracc.org/ns/xtr/1.0:nrefs",NULL } },
  { { "xtr:overlap",NULL } , { "http://oracc.org/ns/xtr/1.0:overlap",NULL } },
  { { "xtr:ref",NULL } , { "http://oracc.org/ns/xtr/1.0:ref",NULL } },
  { { "xtr:refs",NULL } , { "http://oracc.org/ns/xtr/1.0:refs",NULL } },
  { { "xtr:rend-label",NULL } , { "http://oracc.org/ns/xtr/1.0:rend-label",NULL } },
  { { "xtr:rows",NULL } , { "http://oracc.org/ns/xtr/1.0:rows",NULL } },
  { { "xtr:se_label",NULL } , { "http://oracc.org/ns/xtr/1.0:se_label",NULL } },
  { { "xtr:silent",NULL } , { "http://oracc.org/ns/xtr/1.0:silent",NULL } },
  { { "xtr:span",NULL } , { "http://oracc.org/ns/xtr/1.0:span",NULL } },
  { { "xtr:spanall",NULL } , { "http://oracc.org/ns/xtr/1.0:spanall",NULL } },
  { { "xtr:sref",NULL } , { "http://oracc.org/ns/xtr/1.0:sref",NULL } },
  { { "xtr:standalone",NULL } , { "http://oracc.org/ns/xtr/1.0:standalone",NULL } },
  { { "xtr:type",NULL } , { "http://oracc.org/ns/xtr/1.0:type",NULL } },
  { { "xtr:unit",NULL } , { "http://oracc.org/ns/xtr/1.0:unit",NULL } },
  { { "xtr:uref",NULL } , { "http://oracc.org/ns/xtr/1.0:uref",NULL } },
  { { NULL,NULL } , { NULL,NULL } },
};
struct xnn_xname asl_enames[] =
{
  { "g:a", "http://oracc.org/ns/gdl/1.0:a" },
  { "g:b", "http://oracc.org/ns/gdl/1.0:b" },
  { "g:c", "http://oracc.org/ns/gdl/1.0:c" },
  { "g:d", "http://oracc.org/ns/gdl/1.0:d" },
  { "g:f", "http://oracc.org/ns/gdl/1.0:f" },
  { "g:g", "http://oracc.org/ns/gdl/1.0:g" },
  { "g:gg", "http://oracc.org/ns/gdl/1.0:gg" },
  { "g:gloss", "http://oracc.org/ns/gdl/1.0:gloss" },
  { "g:m", "http://oracc.org/ns/gdl/1.0:m" },
  { "g:n", "http://oracc.org/ns/gdl/1.0:n" },
  { "g:nonw", "http://oracc.org/ns/gdl/1.0:nonw" },
  { "g:o", "http://oracc.org/ns/gdl/1.0:o" },
  { "g:p", "http://oracc.org/ns/gdl/1.0:p" },
  { "g:q", "http://oracc.org/ns/gdl/1.0:q" },
  { "g:r", "http://oracc.org/ns/gdl/1.0:r" },
  { "g:s", "http://oracc.org/ns/gdl/1.0:s" },
  { "g:surro", "http://oracc.org/ns/gdl/1.0:surro" },
  { "g:swc", "http://oracc.org/ns/gdl/1.0:swc" },
  { "g:t", "http://oracc.org/ns/gdl/1.0:t" },
  { "g:v", "http://oracc.org/ns/gdl/1.0:v" },
  { "g:w", "http://oracc.org/ns/gdl/1.0:w" },
  { "g:x", "http://oracc.org/ns/gdl/1.0:x" },
  { "n:g", "http://oracc.org/ns/norm/1.0:g" },
  { "n:grouped-word", "http://oracc.org/ns/norm/1.0:grouped-word" },
  { "n:s", "http://oracc.org/ns/norm/1.0:s" },
  { "n:w", "http://oracc.org/ns/norm/1.0:w" },
  { "n:word-group", "http://oracc.org/ns/norm/1.0:word-group" },
  { "sl:aka", "http://oracc.org/ns/sl/1.0:aka" },
  { "sl:base", "http://oracc.org/ns/sl/1.0:base" },
  { "sl:cpds", "http://oracc.org/ns/sl/1.0:cpds" },
  { "sl:ctnd", "http://oracc.org/ns/sl/1.0:ctnd" },
  { "sl:ctnr", "http://oracc.org/ns/sl/1.0:ctnr" },
  { "sl:finl", "http://oracc.org/ns/sl/1.0:finl" },
  { "sl:form", "http://oracc.org/ns/sl/1.0:form" },
  { "sl:formproxy", "http://oracc.org/ns/sl/1.0:formproxy" },
  { "sl:h", "http://oracc.org/ns/sl/1.0:h" },
  { "sl:homophones", "http://oracc.org/ns/sl/1.0:homophones" },
  { "sl:i", "http://oracc.org/ns/sl/1.0:i" },
  { "sl:iheader", "http://oracc.org/ns/sl/1.0:iheader" },
  { "sl:images", "http://oracc.org/ns/sl/1.0:images" },
  { "sl:info", "http://oracc.org/ns/sl/1.0:info" },
  { "sl:inherited", "http://oracc.org/ns/sl/1.0:inherited" },
  { "sl:init", "http://oracc.org/ns/sl/1.0:init" },
  { "sl:inote", "http://oracc.org/ns/sl/1.0:inote" },
  { "sl:iv", "http://oracc.org/ns/sl/1.0:iv" },
  { "sl:lemma", "http://oracc.org/ns/sl/1.0:lemma" },
  { "sl:lemmas", "http://oracc.org/ns/sl/1.0:lemmas" },
  { "sl:letter", "http://oracc.org/ns/sl/1.0:letter" },
  { "sl:list", "http://oracc.org/ns/sl/1.0:list" },
  { "sl:listdef", "http://oracc.org/ns/sl/1.0:listdef" },
  { "sl:lit", "http://oracc.org/ns/sl/1.0:lit" },
  { "sl:lref", "http://oracc.org/ns/sl/1.0:lref" },
  { "sl:medl", "http://oracc.org/ns/sl/1.0:medl" },
  { "sl:memb", "http://oracc.org/ns/sl/1.0:memb" },
  { "sl:name", "http://oracc.org/ns/sl/1.0:name" },
  { "sl:note", "http://oracc.org/ns/sl/1.0:note" },
  { "sl:pname", "http://oracc.org/ns/sl/1.0:pname" },
  { "sl:proof", "http://oracc.org/ns/sl/1.0:proof" },
  { "sl:q", "http://oracc.org/ns/sl/1.0:q" },
  { "sl:qs", "http://oracc.org/ns/sl/1.0:qs" },
  { "sl:sign", "http://oracc.org/ns/sl/1.0:sign" },
  { "sl:signlist", "http://oracc.org/ns/sl/1.0:signlist" },
  { "sl:signs", "http://oracc.org/ns/sl/1.0:signs" },
  { "sl:smap", "http://oracc.org/ns/sl/1.0:smap" },
  { "sl:sref", "http://oracc.org/ns/sl/1.0:sref" },
  { "sl:sys", "http://oracc.org/ns/sl/1.0:sys" },
  { "sl:sysdef", "http://oracc.org/ns/sl/1.0:sysdef" },
  { "sl:uage", "http://oracc.org/ns/sl/1.0:uage" },
  { "sl:ucun", "http://oracc.org/ns/sl/1.0:ucun" },
  { "sl:uname", "http://oracc.org/ns/sl/1.0:uname" },
  { "sl:unote", "http://oracc.org/ns/sl/1.0:unote" },
  { "sl:v", "http://oracc.org/ns/sl/1.0:v" },
  { "xh:div", "http://www.w3.org/1999/xhtml:div" },
  { "xh:h1", "http://www.w3.org/1999/xhtml:h1" },
  { "xh:h2", "http://www.w3.org/1999/xhtml:h2" },
  { "xh:h3", "http://www.w3.org/1999/xhtml:h3" },
  { "xh:innerp", "http://www.w3.org/1999/xhtml:innerp" },
  { "xh:p", "http://www.w3.org/1999/xhtml:p" },
  { "xh:span", "http://www.w3.org/1999/xhtml:span" },
  { "xtr:l2t", "http://oracc.org/ns/xtr/1.0:l2t" },
  { "xtr:map", "http://oracc.org/ns/xtr/1.0:map" },
  { "xtr:translation", "http://oracc.org/ns/xtr/1.0:translation" },
  { NULL, NULL },
};
struct xnn_data asl_data = { asl_enames, asl_anames, asl_nstab };
