...
static void *
hash_insert(unsigned char *keyp, void *datap, Hash*tablep)
{
  if (!hash_find(tablep,keyp))
    hash_add(tablep,pool_copy(keyp, atfmp->pool),datap);
  return datap;
}
...
sH(void *userData, const char *name, const char **atts)
{
  if (name[0] == 'l' && name[1] == '\0' && loading_links)
      ...
      if (xmlid && label) /* lgs doesn't have id/label */
	{
	  ...
	  hash_insert((ucp)hashid,(unsigned char *)buf,label_table);
	}
    }
