...
lpt_save_anchor(unsigned char *c, struct xcl_l*lp, int err_lnum)
{
  ...
  *e = '\0';
  if (hash_find(lp->xc->lpt_anchors,c))
    ...
    hash_add(lp->xc->lpt_anchors,pool_copy(c, lp->xc->pool),(void*)lp->xml_id);
  *e = ec;
}
	...
	case '$':
	  {
		...
		*end = '\0';
		...
		if (longprop_val)
		  ...
		  add_lp(&lp, LPC_property, LPT_short_prop, (unsigned char*)kp->k, bracketing_level);
	      }
	    else
	      {
...
static void
process_boundaries(Node *np, struct xcl_context*xc,struct ilem_para *p,int pos)
{
  struct ilem_para *pp;
...
ilem_para_boundaries(struct xcl_l*lp, struct xcl_context*xc)
{
  if (xc && lp)
    {
      if (lp->ante_para)
	process_boundaries(lp->np, xc,lp->ante_para,1);
      if (lp->post_para)
	process_boundaries(lp->np, xc,lp->post_para,0);
    }
}
