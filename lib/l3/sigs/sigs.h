...
struct sig_context
{
  Pool *pool;
  struct xpd *xpd;
  Hash *known_bad;/* avoid giving lots of warnings for 
			    one file */
  Hash *langs; 	/* A hash of langtags, values are Lists 
  ...
  Memo *mb_f2s;
  Memo *mb_f2ps;		/* COF support uses this */
  Memo *mb_ilem_forms;
  Memo *mb_sigsets;
  Memo *mb_sigs;
...
struct sigset
{
  ...
  Uchar *fmem;
  Uchar **lines;
  Hash *forms; /* A hash of orthographic forms, values
			...
			the target of a lookup.
		      */
  Hash *norms; /* A hash of citation forms and/or
...
extern struct sig const * const *sigs_inst_in_sigset_pass2(struct xcl_context *xcp, 
						     struct ilem_form *ifp, 
						     struct sigset *sp, int *nfinds);
