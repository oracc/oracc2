...
static Pool *cdlidate_pool;
static int iso_sub(const char *base, const char *year);

void
cdlidate_init(void)
{
  cdlidate_pool = pool_init();
}

void
cdlidate_term(void)
{
  pool_term(cdlidate_pool);
}
...
cdlidate_parse(struct cdlidate*dp, const char *date)
{
  ...
  *dest = '\0';
  ...
  if (!(dot = strrchr(end,'.')))
    return 1;
  *end = '\0';
  dp->ruler = (char*)pool_copy((unsigned char*)dot+1,cdlidate_pool);
  
  *dot = '\0';
  dp->dynasty = (char*)pool_copy((unsigned char*)copy,cdlidate_pool);
