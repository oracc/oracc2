...
/* This routine assumes it has been caller is cbd_key_cgp or
   cbd_key_cgpse */
static void
cbd_key_fields(Form *f, const char *period)
{
  char *insert = buf + strlen(buf);
  sprintf(insert, "%s", period);
  cbdact(buf);
#define fpr(c,x) if(f->x){sprintf(insert,"%c%s",c,f->x); cbdact(buf); } /*(t,vp,qid)*/
  if (f->oform)
    {
...
void
cbd_key_cgp(Form *f, const char *period)
{
  ...
  cbdact(buf);
  cbd_key_fields(f, period);
}

/* This routine assumes it has been called immediately after
   cbd_key_cgp */
void
cbd_key_cgpse(Form *f, const char *period)
{
