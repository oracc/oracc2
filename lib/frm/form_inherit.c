...
#define inherit(memb) if((!(inheritor_f2->memb) || !(*inheritor_f2->memb))&&from_f2->memb)inheritor_f2->memb=from_f2->memb
  ...
  if (!inheritor_f2->cf || (!BIT_ISSET(inheritor_f2->flags, FORM_FLAGS_NOT_IN_SIGS)
			    && from_f2->cf && strcmp((char*)inheritor_f2->cf,(char*)from_f2->cf)))
    {
      inheritor_f2->cf = from_f2->cf;
      BIT_CLEAR(inheritor_f2->flags, FORM_FLAGS_NORM_IS_CF);
    }
  if (!inheritor_f2->gw || (!BIT_ISSET(inheritor_f2->flags, FORM_FLAGS_NOT_IN_SIGS)
			    && from_f2->gw && strcmp((char*)from_f2->gw, "X")
			    && strcmp((char*)inheritor_f2->gw,(char*)from_f2->gw)))
    inheritor_f2->gw = from_f2->gw;
  if (!inheritor_f2->sense || (!BIT_ISSET(inheritor_f2->flags, FORM_FLAGS_NOT_IN_SIGS)
  ...
  if (!inheritor_f2->pos)
    ...
    {
      if (from_f2->pos && !BIT_ISSET(inheritor_f2->flags, FORM_FLAGS_NOT_IN_SIGS))
	{
  ...
  inherit(morph2);
#undef inherit
  BIT_SET(inheritor_f2->flags, FORM_FLAGS_NEWLITERAL);
}
