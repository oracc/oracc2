...
{				/* lists of files for in1+in2 and -[i|x]FILE options */
  FNODEPTR inplink;
  FNODEPTR inclink;
  FNODEPTR exclink;
} FLIST;

typedef union linenum
{				/* line number union */
...
typedef struct wtelem
{				/* the basic word element */
...
{				/* linked list of word elements, for frequency sort */
  struct tnode *wtt;		/* word element structure */
  struct wlist *wlnext;		/* next wlist element */
} WLIST, *WLISTPTR;

typedef struct ntelem
{				/* the basic number element */
...
{				/* the basic node */
  ...
  union
  {				/* structures of types this tree manages */
...
/* xml_output does not provide generic support for XML; it only
   supports -wcl* mode as used for concordancing of text corpora in
   Oracc */
int xml_output = 0;
     ...
     int *wcntptr, exclflag;
{
  ...
  if (exclflag == IEFEXCSKIP && Linemode == 0)
    type = 1;
  else if (exclflag == IEFEXCSKIP)
    type = 2;
  else if (Linemode == 0)
    type = 3;
  else
     ...
     TREEPTR p, pv, pw;
{
  ...
  if ((p == pv->left && pv == pw->right)
      || (p == pv->right && pv == pw->left))
    type = 1;
  else if ((p == pv->left && pv == pw->left)
	   || (p == pv->right && pv == pw->right))
    type = 2;
  else
    {
      ...
      fprintf (stderr, "pv %ld, pv->p %ld, pv->l %ld, pv->r %ld\n",
	       (long) pv, (long) pv->parent, (long) pv->left,
	       (long) pv->right);
      fprintf (stderr, "pw %ld, pw->p %ld, pw->l %ld, pw->r %ld\n", (long) pw,
	       (long) pw->parent, (long) pw->left, (long) pw->right);
      error ("(drotate)", 0);
    }
  if (type == 1)
    {
  ...
  if (pw->parent != NULL)
    {
      if (pw == pw->parent->left)
	pw->parent->left = p;
      else if (pw == pw->parent->right)
	pw->parent->right = p;
      else
	{
	  fprintf (stderr, "Bad pointers in tree:\n");
	  fprintf (stderr, "pw %ld, pw->par->l %ld, pw->par->r %ld",
		   (long) pw, (long) pw->parent->left,
		   (long) pw->parent->right);
	  error ("(drotate 1)", 0);
	}
    }
  pv->parent = p;
     ...
     FLIST *flistp;
{
  ...
  while (*fnamep)
	...
	*s = '\0';
      ...
      if (strchr (fnamep, '*') || strchr (fnamep, '?'))
	{
	  if ((wfl = filedir (fnamep, 0x00)) == NULL)
	      ...
	      for (wf = wfl, fnop = NULL; *wf != NULL;)
		{
		  ...
		  if (plen)
		    strncpy (wfnop->fname, fnamep, plen);
		  strcpy (wfnop->fname + plen, wf);
		  if (fnop == NULL)
		    fnop = wfnop;
		  else
		    {
		      for (wfnext = fnop; wfnext->fnext != NULL;
			   wfnext = wfnext->fnext);
		      wfnext->fnext = wfnop;
		    }
		  wf = wf + strlen (wf) + 1;
	...
	{
	  if (flistp->inclink == NULL)
	    flistp->inclink = fnop;
	  else
	    {
	      for (fnop2 = flistp->inclink; fnop2->fnext != NULL;
		   fnop2 = fnop2->fnext);
	      fnop2->fnext = fnop;
	    }
	}
	...
	{
	  if (flistp->exclink == NULL)
	    flistp->exclink = fnop;
	  else
	    {
	      for (fnop2 = flistp->exclink; fnop2->fnext != NULL;
		   fnop2 = fnop2->fnext);
	      fnop2->fnext = fnop;
	    }
	}
	...
	{
	  if (flistp->inplink == NULL)
	    flistp->inplink = fnop;
	  else
	    {
	      for (fnop2 = flistp->inplink; fnop2->fnext != NULL;
		   fnop2 = fnop2->fnext);
	      fnop2->fnext = fnop;
	    }
	}
     ...
     int lim;
{
  ...
  while (--lim > 0)
      ...
      if (Linemode == 2 ? t == SPACE : t != CDIGIT && c != '.')
	{
      ...
      else if (Linemode != 2 && c == '.')
	--w;
    }
  if ((lim = 0))
    {				/* skip past too long part of line 'numbers' */
      if (Linemode == 2)
     ...
     int val;
{
  ...
  printf
    ("    infile(s) and outfile omitted:  read input from standard input.\n");
#ifdef WILDCARDS
  printf
    ("          All filenames except for outfile can contain wildcards.\n");
#endif
  printf (" -f       Do not include word frequency counts in output.\n");
  printf (" -n       Do not write line number indexes for each word.\n");
  printf
    (" -u       Character conversion to upper case (default is to lower case).\n");
  printf (" -c       No case conversion ('abc' is not the same as 'ABC').\n");
  printf
    (" -w       Define 'word' as any whitespace-bounded string of characters.\n");
  printf
    (" -o (-o+) Sort output by decreasing (increasing) word frequency.\n");
  printf
    (" -l       Use line numbers included in the input file at the beginning of\n");
  printf
    ("          lines of text;  numbers must be entirely digits (%d digits max).\n",
     NUMSIZE - 1);
  printf
    ("          a line not beginning with a number will get the most recent number.\n");
  printf
    ("          (default is to set line numbers by counting newline characters).\n");
  printf
    (" -l*      Like -l, except that the first non-blank text (up to %d characters)\n",
     SNUMSIZE);
  printf
    ("          in each line will be used as the line 'number' for that line.\n");
  printf (" -?       Display this help screen.\n");
  printf (" -iFILE   Only words included in FILE(s) will be indexed.\n");
  printf
    (" -xFILE   Words appearing in FILE(s) will be excluded from the index.");
...
concord_index ()		/* do the indexing */
{
  ...
  if (Linemode == 1)
    ...
    linenp->strnum = (unsigned char *) allocate (SNUMSIZE);
  ...
  for (fnop = Files.exclink; fnop != NULL; fnop = fnop->fnext)
	...
	fileerror ("exclude", fnop->fname);
      ...
      Infile = tmpfile;
      Uniqwords = Wordcount = 0;
      if (Reffile != stdout && Reffile != stdin)
	fclose (Reffile);
      Reffile = NULL;
      Dexcfile = IEFEXCSKIP;
    }
  ...
  for (fnop = Files.inplink; fnop != NULL || Infile == stdin;
       fnop = fnop->fnext)
    {
      if (fnop != NULL)
	if ((Infile = fopen (fnop->fname, "r")) == NULL)
	...
	buildindex (root0, &root1, linenp, &Wordcount, IEFEXCSKIP) :
	buildindex (root0, NULL, linenp, &Wordcount, IEFOFF);
      if (Infile != stdout && Infile != stdin)
	fclose (Infile);
      Infile = NULL;
      if (fnop == NULL)
	break;
      if (fnop->fnext != NULL && Linemode != 2)
	linenp->numnum++;
    }
  ...
  if (!do_silent_running)
    fprintf (stderr,
	     ...
	     Uniqwords, linenp->numnum);
  if (xml_output)
    fputs("<concord>", Outfile);
     ...
     LINENUM *lp;
{
  ...
  if (Linemode == 2)
    {
      p1->lnum.strnum =
	(unsigned char *) allocate (strlen ((char *) lp->strnum) + 1);
  ...
  if (p == NULL || (cond = strcmp (w, p->WTNODE.word)) != 0)
      ...
      if (Doindex)
	{
	  if (p->WTNODE.lines == NULL && Dincfile == IEFINCSKIP)
	    ...
	    {
	      temp = p->WTNODE.lines;
	      while (temp->lnodeptr != NULL && LNUMCOMP (temp->lnum, lp))
		temp = temp->lnodeptr;
     ...
     TREEPTR p;
{
  if (p != NULL)
	...
	{
	  WLISTPTR wlp;
	  for (wlp = p->NTNODE.wfirst; wlp != NULL; wlp = wlp->wlnext)
