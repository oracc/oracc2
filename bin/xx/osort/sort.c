/* sort - sort lines of text (with all kinds of options).
   ...
   Modified by Steve Tinney (stinney@sas.upenn.edu) to support graphemic
   sorts for cuneiform languages 1997-2001.
...
struct keyfield
{
  ...
  int graphemic;		/* Sort graphemically */
  int utf8;		        /* Sort strings using utf8 */
...
/* Flag to request utf8 processing */
static int utf8;
...
\n\
  ...
  -G               compare according to graphemic value\n\
  ...
  -U               character data is in UTF-8 encoding\n\
...
or more of MbdfGinr, this effectively disable global -MbdfGinr settings\n\
...
limfield (const struct line *line, const struct keyfield *key)
{
  ...
  {
    int nchars;
    for (nchars = echar; ptr < lim && nchars; --nchars)
      {
	while (*++ptr & 0x80)
	  ;
      }
...
{
  register unsigned char tmpa = UCHAR (*a);
  register unsigned char tmpb = UCHAR (*b);
...
keycompare (const struct line *a, const struct line *b)
{
  ...
  for (key = keyhead.next; key; key = key->next, ++iter)
      ...
      if (key->graphemic) 
	{
      ...
      else if (key->utf8) /* FIXME: lengths are not correctly handled yet */
	{
	  /* translate is handled magically in this compare */
	  if (*lima || *limb)
	    {
	      char savea = *lima, saveb = *limb;
...
main (int argc, char **argv)
{
  ...
  gkey.ignore = NULL;
  gkey.translate = NULL;
  gkey.graphemic = gkey.numeric =  gkey.general_numeric = gkey.month = gkey.reverse = 0;
  ...
  for (i = 1; i < argc; ++i)
      ...
      if (argv[i][0] == '+')
	{
	  ...
	  key->graphemic = key->numeric = key->general_numeric = key->month = key->reverse = 0;
	  s = argv[i] + 1;
...
key specifiers,\nthe +POS specifier must come first"));
		...
		switch (*s)
		  ...
		  case 'k':
		    ...
		    key->translate = NULL;
		    key->skipsblanks = key->skipeblanks = 0;
		    key->graphemic = key->numeric = key->month = key->reverse = 0;
		    /* Get POS1. */
		    if (!digits[UCHAR (*s)])
		      badfieldspec (argv[i]);
		    for (t = 0; digits[UCHAR (*s)]; ++s)
...
but lacks following character offset"));
  ...
  for (key = keyhead.next; key; key = key->next)
	...
	&& !key->skipeblanks && !key->month && !key->graphemic && !key->numeric
        && !key->general_numeric)
      {
  ...
  if (!keyhead.next && (gkey.ignore || gkey.translate || gkey.skipsblanks
			|| gkey.skipeblanks || gkey.month || gkey.graphemic
