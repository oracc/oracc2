...
#define xpool_copy(x) (char*)pool_copy((unsigned char *)x,pool)
...
Hash *bigrams, *bigram_lang_hashes;
...
shift_sig(char *tmp_sig)
{
  ...
  if ((norm_end = strchr(curr_sig, '$')))
    {
      size_t n = strcspn((char*)norm_end+1,FORM_FIELD_ENDS_STR); /*'*/
      norm_end += n + 1;
      *norm_end = '\0';
    }
  ...
  if (last_sig && !strcmp(last_sig_lang, curr_sig_lang))
  ...
  else if (*curr_sig_lang)
    {
      set_last_sig(curr_sig);
      strcpy(last_sig_lang, curr_sig_lang);
      if (!(bigrams = hash_find(bigram_lang_hashes, (ucp)last_sig_lang)))
	{
	  bigrams = hash_create(1024);
	  hash_add(bigram_lang_hashes, (ucp)xpool_copy(last_sig_lang), bigrams);
	}
    }
}
...
fn_expand(void *p)
{
  fnlist[findex++] = strdup((ccp)expand(iproject, p, ext));
}

static void
marshall_bigrams_2(unsigned char *key, int *data)
{
  bgms[nbgms].bgm = malloc(strlen((ccp)head_bigram) + strlen((ccp)key) + 2);
  sprintf((char*)bgms[nbgms].bgm, "%s %s", head_bigram, key);
  bgms[nbgms].freq = *data;
  ++nbgms;
}

static void
marshall_bigrams(unsigned char *key, Hash*data)
{
  head_bigram = key;
...
static void
dump_bigrams(char *lang, Hash *bhash)
{
...
main(int argc, char * const*argv)
{
  ...
  *last_sig_lang = *curr_sig_lang = '\0';
  ...
  mb_ints = memo_init(sizeof(int), 32);
