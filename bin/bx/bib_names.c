...
void
bnm_all_names(Bibentry *ep, Nameinfo *nip)
{
  int i;
  int len;
  for (i = len = 0; i < nip->nnames; ++i)
    len += strlen(nip->names[i]->nkey)+1;
  char buf[len+1]; *buf = '\0';
  for (i = 0; i < nip->nnames; ++i)
    {
      strcat(buf, nip->names[i]->nkey);
      strcat(buf, "+");
    }
  buf[strlen(buf)-1] = '\0';
  nip->allnames = (ccp)pool_copy((uccp)buf, ep->bib->bp->p);
}
