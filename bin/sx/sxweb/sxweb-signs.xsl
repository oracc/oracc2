...
<xsl:template name="sel-href">
  <xsl:param name="type"/>
  <xsl:value-of select="concat($oracc, '/',/*/@project,
			'/signlist/01bld/selpages/',
...
<xsl:template name="sws-lem-page">
  ...
  <xsl:variable name="href">
    <xsl:call-template name="sel-href">
      <xsl:with-param name="type" select="'sux'"/>
    </xsl:call-template>
  </xsl:variable>
  <!--<xsl:message>sws-lem-page writing <xsl:value-of select="$href"/></xsl:message>-->
  <ex:document href="{$href}"
...
<xsl:template name="sws-lex-page">
  ...
  <xsl:if test="$asl-lexdata = 'yes'">
    ...
    <xsl:if test="$lex-ok='y'">
      <xsl:variable name="href">
	<xsl:call-template name="sel-href">
	  <xsl:with-param name="type" select="'lex'"/>
	</xsl:call-template>
      </xsl:variable>
      <ex:document href="{$href}"
		   method="xml" encoding="utf-8" omit-xml-declaration="yes"
