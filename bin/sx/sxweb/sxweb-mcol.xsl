<xsl:transform 
  ...
  <!-- This is a custom version of mcol.xsl for use by sxweb-overview.xsl and sxweb-letters.xsl.
       This version ignores @fake and @compoundonly signs.-->
...
<xsl:template name="mcol">
  ...
  <table>
    ...
    <xsl:for-each select="$nodes[local-name()=$tag][not(@fake)][not(@compoundonly)][(position() - 1) mod $columns = 0]">
...
<xsl:template match="*" mode="mcol-first">
  ...
  <tr>
    <xsl:apply-templates mode="mcol"
			 select=".|following-sibling::*[local-name()=$tag][not(@fake)][not(@compoundonly)][position() &lt; $columns]"/>
    <xsl:if test="count(following-sibling::*[not(@fake)]) &lt; ($columns - 1)">
      <xsl:call-template name="emptycell">
        <xsl:with-param name="cells" select="$columns - 1 - count(following-sibling::*[local-name()=$tag][not(@fake)][not(@compoundonly)])"/>
      </xsl:call-template>
    </xsl:if>
  </tr>
</xsl:template>
