...
static void
tex_between_outers(void)
{
}

static void
tex_after_outers(void)
{
...
static void
tex_before_block(void)
{
  P0("\\BeginBlock\n");
}

static void
tex_after_block(void)
{
  P0("\\EndBlock\n");
}

static void
tex_before_sources(void)
{
}

static void
tex_between_sources(void)
{
}

static void
tex_after_sources(void)
{
}
...
static void
tex_each_composite_column(void)
{
...
static void
tex_between_composite_columns(void)
{
  if (!iterate_composite_column->zero_flag 
      || (iterate_composite_column->complex->complex_has_variant
...
static void
tex_before_reconstructed(void)
{
  tex_cmp_phantoms = 0;
}

static void
tex_after_reconstructed(void)
{
  P0("\\cr\n");
  if (tex_template_phantoms != tex_cmp_phantoms)
    {
...
static void
tex_each_reconstructed_column(void)
{
...
static void
tex_between_reconstructed_columns(void)
{
}

static void
tex_before_source(void)
{
  tex_src_phantoms = tex_output_col_count = tex_zeroes_skipped = 0;
}

static void
tex_after_source(void)
{
  P0("\\cr\n");
      ...
      if (*tht_txt)
	{
	  ...
	  *dest = '\0';
	  if (breakage == 1)
	    {
	      Uchar *dst, *tmp;
	      strcat((char*)buf, "\\[");
	      dst = tex_hack_text_buf + strlen((char*)tex_hack_text_buf);
		...
		*dst++ = *tmp;
	      ...
	      *dst++ = *tmp++;
	      strcpy((char*)dst, "");
	      strcat((char*)dst, (ccp)tmp);
	      strcat((char*)dst,"\\]");
	    }
	  else if (breakage == 2)
	    {
	      strcat((char*)buf, "\\brackets{");
	      strcat((char*)buf, (ccp)tht_txt);
	    }
	  else
	    {
	      strcat((char*)buf, (ccp)tht_txt);
	  ...
	  for (tmp = tht_meta; *tmp; ++tmp)
	    strcat((char*)buf, (ccp)tex_meta(*tmp));
	  if (breakage == 2)
	    strcat((char*)buf, "}");
	  strcat((char*)buf, (ccp)tht_delim);
	}
      else
	break;
    }
...
tex_build_ic (Source_column*sp, Uchar *cs)
{
  ...
  if ('~' == *sp->text)
    ++sp->text;
  if (cs)
    sprintf ((char*)ic, "\\%s{%s}", cs, sp->text);
  else
    {
      sprintf ((char*)ic, "\\%cic{%s}{\\hss %s\\hss}",
...
tex_expand_pre (int cols, int phantoms, Uchar *txt)
{
  if ('[' == *txt)
	...
	delim = (ucp)"";
      sprintf ((char*)expand_pre + strlen((ccp)expand_pre),
...
tex_each_source_column(void)
{
  ...
  if (do_expand 
      ...
      else
	{
	  ...
	  *expand_pre = '\0';
	  while (!strncmp((ccp)s, "\\X", 2))
	    {
	      int cols, phant, zeroes, n;
	      Uchar *txt;
	      s += 2;
	      sscanf((ccp)s,"%d/%d/%d/%n", &cols, &phant, &zeroes, &n);
	      s += n;
	      txt = s;
	      if ('[' == *s)
		while (*s && ']' != *s++)
...
#endif
	{
	  ...
	  endp = iterate_source_column->text + strlen((ccp)iterate_source_column->text) - 2;
	  if (sp && '.' == *sp->text && ']' == *endp)
	    {
	      *endp = '\0';
  ...
  if (0 == iterate_source_column->index)
      ...
      if (*expand_pre)
	{
	  if (complex_phantoms > 0)
	    {
	      int cols, phant, n;
	      sscanf ((ccp)expand_pre, "&\\Ms{%d}\\phantoms{%d}%n", &cols, &phant, &n);
      ...
      if (iterate_source_column->text_entry_flag)
	{
	  ...
	  if (!do_expand
	    ...
	    {
	      append = tex_build_ic (iterate_source_column->right, (ucp)"Mappend");
...
static void
tex_between_source_columns(void)
{
}

static void
tex_before_notes(void)
{
}
...
tex_hack_tabloc (Uchar *s)
{
  ...
  while (1+(2*strlen((ccp)s)) > hack_buf_len)
    {
      hack_buf_len *= 2;
