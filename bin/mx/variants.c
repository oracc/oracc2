...
#define vars_add(s,d)   hash_add(variants_hash,(uccp)xstrdup((ccp)s),(d))
...
{
  sprintf ((char*)vars_key_buf, "%s", vp->block_name);
  return vars_key_buf;
}
...
{
  sprintf ((char*)vars_key_buf, "%s:%d", vp->block_name, vp->begin);
  return vars_key_buf;
}
...
{
  sprintf ((char*)vars_key_buf, "%s:%d:%s", vp->block_name, vp->begin, vp->siglum_name);
  return vars_key_buf;
}
...
{
  sprintf ((char*)vars_key_buf, "%s", block_name);
...
{
  sprintf ((char*)vars_key_buf, "%s:%d", block_name, col);
  return hash_find (variants_hash, vars_key_buf);
}
...
{
  sprintf ((char*)vars_key_buf, "%s:%d:%s", block_name, col, siglum_name);
  return hash_find (variants_hash, vars_key_buf);
}
...
vars_get_variant_text (Variant *vp)
{
  ...
  *vars_variant_text_buf = '\0';
  while (sp)
    {
      strcat ((char*)vars_variant_text_buf, (ccp)sp->text);
      if (sp == vp->sp_end)
	break;
      else
	{
	  strcat ((char*)vars_variant_text_buf, " ");
	  sp = sp->right;
	}
    }
...
vars_get_expanded_variant_text (Variant *vp)
{
  ...
  *vars_variant_text_buf = '\0';
  while (sp)
	...
	{
	  strcat ((char*)vars_variant_text_buf, "0");
	}
      else if (*sp->text == '+' || *sp->text == '*')
	{
	  strcat ((char*)vars_variant_text_buf, (ccp)sp->composite->text);
	}
      else if (*sp->text == ',')
	{
	  strcat ((char*)vars_variant_text_buf, "x");
	}
	...
	break;
	       ...
	       && !sp->composite->zero_flag)
	strcat ((char*)vars_variant_text_buf, (ccp)sp->composite->delim);
      sp = sp->right;
    }
  return vars_variant_text_buf;
}
...
vars_get_main_text (Variant *vp)
{
  ...
  *vars_main_text_buf = '\0';
  while (sp)
	...
	strcat ((char*)vars_main_text_buf, (ccp)sp->composite->text);
      if (sp == vp->sp_end)
	break;
      else
	{
	  if (!sp->composite->zero_flag && '\n' != *sp->composite->delim)
	    strcat ((char*)vars_main_text_buf, (ccp)sp->composite->delim);
	  sp = sp->right;
	}
    }
