...
expand_init_absences (void)
{
  ...
  sprintf ((char*)expand_buf + so_far, "\\X%d/%d/%d/ ", 
	   init_absences, init_phantoms, init_zeroes);
  so_far = strlen((ccp)expand_buf);
  init_absences = init_phantoms = init_zeroes = 0;
}

static void
expand_init_breaks (void)
{
...
expand_complex (Source_column *sp)
{
  ...
  for (target_complex = sp->composite->complex; 
       ...
       sp2 = sp2->right)
    {
      Uchar *txt = (ucp)"", *txt2 = (ucp)"", *meta = (ucp)"";
      const Uchar *delim = (ucp)"";
	...
	delim = sp2->right->delim;
      ...
      if (*delim == ' ')
	delim = (ucp)"\\sp{}";
      new_len = strlen((ccp)txt) + strlen((ccp)txt2) + strlen((ccp)meta) + strlen((ccp)delim);
      while (expand_buf_len <= so_far + new_len + 1 + kernTn_len)
	{
	  expand_buf_len += 128;
	  expand_buf = xrealloc ((char*)expand_buf, expand_buf_len);
	}
      if ('[' == *txt)
	{
	  Uchar *t;
	       ...
	       || sp2->right->composite->complex != sp2->composite->complex)
	{
      ...
      sprintf ((char*)expand_buf + so_far, "%s%s%s%s", txt, txt2, meta, delim);
      so_far += new_len;
    }
  ...
  if (init_absences && !state)
    ...
    expand_init_breaks();
  s = expand_buf + strlen((ccp)expand_buf);

  while (s > expand_buf && (s[-1] == ' ' || s[-1] == '-'))
    --s;
  strcpy ((char*)s, (ccp)kernTn);
  return expand_buf;
}
...
expand_composite_complex (Composite_column *sp)
{
  ...
  s = expand_buf + strlen((char*)expand_buf);
  while (s > expand_buf && (s[-1] == ' ' || s[-1] == '-'))
    --s;
  *s = '\0';
  return expand_buf;
