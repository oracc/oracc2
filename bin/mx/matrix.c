...
scan_input (int ac, char **av)
{
  ...
  while (optind < ac || do_stdin)
      ...
      if (fragments)
	{
      ...
      size_t nb;
      while (NULL != (lp = loadoneline (fp, &nb)))
	{
	  ++lnum;
	  ...
	  if (matches_matrix(lp)) 
	      ...
	      else if (in_block) 
		{
		  if (isspace (*lp) || '#' == *lp)
		    {
		      list_add (curr_notes, (void*)xstrdup ((ccp)lp));
		      last_was_note = TRUE;
		    }
...
new_composite (Block *bp, Uchar *lp)
{
  ...
  if (RECONSTRUCTED_LINE(lp))
    {
      curr_line->name = (ucp)xstrdup((ccp)"");
    }
  else 
    {
      for (s = lp; *s && *s != '.'; ++s)
  ...
  while (NULL != (ep = get_composite_entry (&dp)))
	...
	curr_comp_col->text = (ucp)xstrdup((ccp)ep);
      else
	curr_comp_col->text = (ucp)xstrdup((ccp)atf2utf(NULL,ep,0));
	...
	curr_comp_col->space_flag = FALSE;
      ...
      curr_comp_col->width = strlen ((ccp)ep);
      ...
      if ('(' == *t) /* we have an alternate siglum */
	{
	  ...
	  *t++ = '\0';
	  ...
	  if ('\0' == *t)
	    ...
	    *t++ = '\0';
	  curr_line->altsig = (ucp)xstrdup((ccp)sigp);
	}
      else
	  ...
	  if ('~' == *curr_sou_col->text)
	    {
	      ...
	      if (NULL != (tmp = (ucp)strchr(".-_", curr_sou_col->text[1])))
		{
  ...
  if (ep)
    {
      ...
      if ((carat = (ucp)strchr((ccp)ep,'^')) && isdigit(carat[1]))
	{
	  Uchar *end = carat+1;
	  ...
	  if ('^' == *end)
	    {
	      *carat++ = *end = '\0';
	      curr_sou_col->notemark = (ucp)xstrdup((ccp)carat); /* FIXME: multiple notemarks? */
	    }
	}

      /* we assume that visible_width is for use by the pf-align module */

      if (NULL != (bs = (ucp)strrchr((ccp)ep, '\\')))
	{
	  Uchar *dest, *src;
	...
	curr_sou_col->explicit_ncols = FALSE;
      ...
      if ('\n' == ep[strlen((ccp)ep)-1])
	ep[strlen((ccp)ep)-1] = '\0';

      if ('@' == *ep)
	{
	  curr_sou_col->align_override_flag = TRUE;
	  curr_sou_col->text = (ucp)xstrdup((ccp)atf2utf(NULL, (uccp)ep + 1,0));
	}
      else
	{
	  if (!curr_sou_col->index)
	    curr_sou_col->text = (ucp)xstrdup((ccp)ep);
	  else
	    curr_sou_col->text = (ucp)xstrdup((ccp)atf2utf(NULL,ep,0));
	}
      ...
      if (!curr_sou_col->index)
	{
	  if (curr_line->altsig && *curr_line->altsig)
	    curr_sou_col->visible_width += strlen((ccp)curr_line->altsig) + 2;
	  if (*curr_line->tabloc->compressed)
	    curr_sou_col->visible_width += strlen((ccp)curr_line->tabloc->compressed) + 1;
	}
    }
  else
    {
      curr_sou_col->text = (ucp)xstrdup ("");
...
{
  if (NULL != (s = (ucp)strrchr ((ccp)s, '&')) && isdigit (*(s+1)))
    {
